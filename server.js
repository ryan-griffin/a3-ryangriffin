...
const validate = (input) => {
	...
	if (!assignment.title || assignment.title.length > 100) {
		throw new Error("Assignment name must be between 1 and 100 characters.");
	}
...
app.get("/api/assignments", requireAuth, async (request, response) => {
	try {
		...
		return response.json({ assignments: await assignmentsForUser(username) });
	} catch (error) {
		console.error(error);
		return response.status(500).json({ error: "The request could not be completed." });
	}
...
app.post("/api/assignments", requireAuth, async (request, response) => {
	try {
		...
		return response.status(201).json({ assignments: await assignmentsForUser(username) });
	} catch (error) {
		console.error(error);
		return response.status(400).json({ error: error.message || "The request could not be completed." });
	}
});
